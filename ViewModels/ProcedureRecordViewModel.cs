...
{
    public class ProcedureRecordViewModel: ViewModelBase, INotifyPropertyChanged
    {
        //Fields
        private IEnumerable<ProcedureRecordView> _procedureRecordList;
        ...
        private bool _procedureRecordListVisibility = true;
        ...
        public bool ProcedureRecordListVisibility
        {
            get
            {
                return _procedureRecordListVisibility;
            }

            set
            {
                _procedureRecordListVisibility = value;
                OnPropertyChanged(nameof(ProcedureRecordListVisibility));
            }
        }

        public IEnumerable<ProcedureRecordView> ProcedureRecordList
        {
            get
            {
                return _procedureRecordList;
            }
            set
            {
                _procedureRecordList = value;
                OnPropertyChanged(nameof(ProcedureRecordList));
            }
        }
        ...
        public ICommand UpdateProcedureRecordCommand { get; }
        public ICommand CreateProcedureRecordCommand { get; }
        public ICommand GetProcedureRecordDetails { get; }
        public ICommand ResetCommand { get; }
        public ICommand CancelCommand { get; }
        public ICommand BackToListCommand { get; }
        public ICommand DeleteCommand { get; }

        public ProcedureRecordViewModel()
        {
            ProcedureRecordList = new List<ProcedureRecordView>();
            ...
            LoadProcedureRecordList(SearchWord);
            ...
            GetProcedureRecordDetails = new RelayCommand(ExecuteGetProcedureRecordDetailsCommand, CanExecuteGetUserDetailsCommand);
            UpdateProcedureRecordCommand = new RelayCommand(ExecuteUpdateCommand, CanExecuteForAdminsCommand);
            CreateProcedureRecordCommand = new RelayCommand(ExecuteCreateCommand, CanExecuteCreateCommand);
            ResetCommand = new RelayCommand(ExecuteGetProcedureRecordDetailsCommand);
        ...
        {
            var oldData = procedureRecordRepository.GetByID(SelectedId);
            var oldMedicine = productMedicalRecordRepository.GetAll(SelectedId, RecordTypeEnum.Procedure);
            var deleteSuccess = procedureRecordRepository.Remove(SelectedId);
            if (deleteSuccess)
            {
        ...
        private void ExecuteBackToListCommand(object obj)
        {
            ...
            ProcedureRecordListVisibility = true;
        ...
        {
            if (string.IsNullOrWhiteSpace(ChitNumber) || string.IsNullOrWhiteSpace(ProcedureBill.ToString()) || string.IsNullOrWhiteSpace(TotalBill.ToString()) ||
                string.IsNullOrWhiteSpace(Patient) || string.IsNullOrWhiteSpace(Doctor))
            {
                ...
                PharmacyBillErrorMessage = (string.IsNullOrWhiteSpace(ProcedureBill.ToString())) ? "Procedure bill is required." : "";
            ...
            else
            {
                ...
                ProcedureRecord createProcedureRecord  = new ProcedureRecord()
                {
                    ChitNumber = ChitNumber,
                    ProcedureBill = ProcedureBill,
                    OPDCharge = OPDCharge,
                ...
                var createSuccess = procedureRecordRepository.Add(createProcedureRecord, medDoseIds);
                if (createSuccess)
                {
                    UpdateGridVisibility = false;
                    ProcedureRecordListVisibility = true;
        ...
        private void ExecuteGetProcedureRecordDetailsCommand(object obj)
        {
            ProcedureRecord procedureRecord = procedureRecordRepository.GetByID(SelectedId);
            IEnumerable<ProductMedicalRecord> medicalDose = productMedicalRecordRepository.GetAll(SelectedId, RecordTypeEnum.Procedure);
            var medArray = new[]
            {
            ...
            ChitNumber = procedureRecord.ChitNumber;
            ProcedureBill = procedureRecord.ProcedureBill;
            OPDCharge = procedureRecord.OPDCharge ?? 0;
            OtherCharges = procedureRecord.OtherCharges ?? 0;
            AdmitDate = procedureRecord.AdmitDate;
            TotalBill = procedureRecord.TotalBill;
            ConsultantFee = procedureRecord.ConsultantFee ?? 0;
            Patient = PatientList[procedureRecord.PatientId];
            Doctor = DoctorList[procedureRecord.DoctorId];
            AddedBy = EmployeeList[procedureRecord.AddedBy];
            Nurse1 = procedureRecord.Nurse1Id != null ? NurseList[procedureRecord.Nurse1Id.Value] : string.Empty;
            Nurse2 = procedureRecord.Nurse2Id != null ? NurseList[procedureRecord.Nurse2Id.Value] : string.Empty;
            DocComm = procedureRecord.DocComm;
            Nurse1Comm = procedureRecord.Nurse1Comm;
            Nurse2Comm = procedureRecord.Nurse2Comm;
            for (int i=0; i<medArray.Count(); i++)
            {
        ...
        private void ExecuteUpdateCommand(object obj)
        {
            ...
            else
            {
                ...
                ProcedureRecord updateProcedureRecord = new ProcedureRecord()
                {
                    Id = SelectedId,
                    ChitNumber = ChitNumber,
                    ProcedureBill = ProcedureBill,
                ...
                var oldData = procedureRecordRepository.GetByID(updateProcedureRecord.Id);
                var oldMedicine = productMedicalRecordRepository.GetAll(updateProcedureRecord.Id, RecordTypeEnum.Procedure);
                bool editSuccess = procedureRecordRepository.Edit(updateProcedureRecord, medDoseIds);
                if (editSuccess)
                {
                    var newMedicine = productMedicalRecordRepository.GetAll(updateProcedureRecord.Id, RecordTypeEnum.Procedure);
        ...
        private bool CanExecuteCreateCommand(object obj)
        {
            ...
            ProcedureBill = Total1 + Total2 + Total3 + Total4 + Total5 + Total6 + Total7 + Total8 + Total9 + Total10 + Total11 + Total12;
            TotalBill = OPDCharge + ProcedureBill + OtherCharges + ConsultantFee + docCommission + nurse1Commission + nurse2Commission;
        ...
        private void LoadProcedureRecordList(string searchWord)
        {
            var procedureRecords = procedureRecordRepository.GetAll(searchWord);
            ProcedureRecordList = procedureRecords;
