...
{
    public class MedicalRecordViewModel: ViewModelBase, INotifyPropertyChanged
    {
        //Fields
        private IEnumerable<MedicalRecordView> _medicalRecordList;
        ...
        private bool _medicalRecordListVisibility = true;
        ...
        public bool MedicalRecordListVisibility
        {
            get
            {
                return _medicalRecordListVisibility;
            }

            set
            {
                _medicalRecordListVisibility = value;
                OnPropertyChanged(nameof(MedicalRecordListVisibility));
            }
        }

        public IEnumerable<MedicalRecordView> MedicalRecordList
        {
            get
            {
                return _medicalRecordList;
            }
            set
            {
                _medicalRecordList = value;
                OnPropertyChanged(nameof(MedicalRecordList));
            }
        }
        ...
        public ICommand CreateMedicalRecordCommand { get; }
        public ICommand GetMedicalRecordDetails { get; }
        ...
        public MedicalRecordViewModel()
        {
            ...
            GetMedicalRecordDetails = new RelayCommand(ExecuteGetMedicalRecordDetailsCommand, CanExecuteGetUserDetailsCommand);
            UpdateMedicalRecordCommand = new RelayCommand(ExecuteUpdateCommand, CanExecuteForAdminsCommand);
            CreateMedicalRecordCommand = new RelayCommand(ExecuteCreateCommand, CanExecuteCreateCommand);
            ResetCommand = new RelayCommand(ExecuteGetMedicalRecordDetailsCommand);
        ...
        private void ExecuteGetMedicalRecordDetailsCommand(object obj)
        {
            MedicalRecord medicalRecord = medicalRecordRepository.GetByID(SelectedId);
            IEnumerable<ProductMedicalRecord> medicalDose = productMedicalRecordRepository.GetAll(SelectedId, RecordTypeEnum.Medical);
            var medArray = new[]
            {
            ...
            ChitNumber = medicalRecord.ChitNumber;
            PharmacyBill = medicalRecord.PharmacyBill;
            OPDCharge = medicalRecord.OPDCharge ?? 0;
            OtherCharges = medicalRecord.OtherCharges ?? 0;
            AdmitDate = medicalRecord.AdmitDate;
            TotalBill = medicalRecord.TotalBill;
            Patient = PatientList[medicalRecord.PatientId];
            Doctor = DoctorList[medicalRecord.DoctorId];
            AddedBy = EmployeeList[medicalRecord.AddedBy];
            ConsultantFee = medicalRecord.ConsultantFee ?? 0;
            Nurse1 = medicalRecord.Nurse1Id != null ? NurseList[medicalRecord.Nurse1Id.Value] : string.Empty;
            Nurse2 = medicalRecord.Nurse2Id != null ? NurseList[medicalRecord.Nurse2Id.Value] : string.Empty;
            DocComm = medicalRecord.DocComm;
            Nurse1Comm = medicalRecord.Nurse1Comm;
            Nurse2Comm = medicalRecord.Nurse2Comm;
            for (int i=0; i<medArray.Count(); i++)
            {
        ...
        private void LoadMedicalRecordList(string searchWord)
        {
            var medicalRecords = medicalRecordRepository.GetAll(searchWord);
